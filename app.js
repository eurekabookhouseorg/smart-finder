...
// ==========================================
// 15. VIRTUAL TOUCHSCREEN KEYBOARD (COMPACT 3-ROW)
// ==========================================
...
function setupVirtualKeyboard() {
  ...
  if (currentKbMode === "abc") {
    ...
    // Baris 2: ASDFGHJKL + Tombol SPASI di kanan
    r2.innerHTML =
      row2.map(makeKey).join("") +
      `<button type="button" onclick="kbInput(' ')" class="kb-key-btn action-btn btn-space" title="Spasi">
        <span>SPASI</span>
      </button>`;

    // Baris 3: [?123] di kiri + ZXCVBNM + [HAPUS] + [CARI] di kanan
    r3.innerHTML =
    ...
    // Baris 2: Simbol Populer + SPASI di kanan
    r2.innerHTML =
      row2.map(makeKey).join("") +
      `<button type="button" onclick="kbInput(' ')" class="kb-key-btn action-btn btn-space" title="Spasi">
        <span>SPASI</span>
    ...
    // Baris 3: [ABC] di kiri + Simbol + [HAPUS] + [CARI] di kanan
    r3.innerHTML =
